...
export default {
  ...
  mounted() {
    ...
    this.onSelcet(this.leftRightlAngleSelect);
  },
  computed: {
    ...mapState({
    // 左右扫风
      leftRightlAngleSelect: state => {
        return state.publicInfo.attr.leftRightlAngleSelect
      }
    })
  },
  methods: {
    ...
    onSelcet(val) {
    //  右定格：6关闭：0当前位置定格：1偏左定格：3左定格：2偏右定格：5中定格：4
      switch (val) {
        case 0:
    ...
    handle(e) {
      ...
      // console.error('screen-ponit:', e.touches[0].screenX, e.touches[0].screenY);
      // console.error('page-ponit:', e.touches[0].pageX, e.touches[0].pageY);
      if (y < center.y) {
        // 在扇形上侧区域
      ...
      if (this.getDirc({x, y}, center) > radius) {
        // 在扇形左侧区域，但与圆心距离过远
        // console.log('this.getDirc({x, y}, center) > radius');
        // TODO
        return;
      }
      ...
      const distanceToCenterY = y - center.y;
      const arc = Math.asin(distanceToCenterY / this.getDirc({x, y}, center));
      // console.error('getDirc({x,y},center)',getDirc({x,y},center), distanceToCenterX)
      // console.error('arc',arc)
      let ancr = arc * 180 / Math.PI;
      // console.error('ancr before', ancr)
    ...
    handleTouchEnd(e) {
      ...
      if (selectLength === 5) {
        this.$store.dispatch('setDeviceStatus', {
          leftRightlAngleSelect: 1
        });
      } else if (selectLength === 0) {
        this.$store.dispatch('setDeviceStatus', {
          leftRightlAngleSelect: 0
        });
      } else if (selectLength === 1) {
        let selectIdx = this.show.indexOf(1);
        this.$store.dispatch('setDeviceStatus', {
          leftRightlAngleSelect: values[selectIdx]
        });
      } else {
        this.showToast();
      }
  ...
  watch: {
    'leftRightlAngleSelect': function (val, oldVal) {
      this.onSelcet(val);
    }
  }
}
...
.bgImg {
  ...
  list-style: none;
  overflow: hidden;
  transform: rotate(-135deg); /*针对mac safari浏览器兼容*/
  border-radius: 234px;
}
